...
use warp::{Filter, http::StatusCode, reply::Reply, { http::header::LOCATION }};
...
impl From<&str> for ResponseMessage {
    fn from(message: &str) -> ResponseMessage {
        ResponseMessage { message: message.to_string() }
    }
}
...
impl ResponseMessage {
    pub fn into_response(self, status_code: StatusCode) -> Result<warp::reply::WithStatus<warp::reply::Json>, Infallible> {
        Ok(self.into_raw_response(status_code))
    }

    pub fn into_raw_response(self, status_code: StatusCode) -> warp::reply::WithStatus<warp::reply::Json> {
...
pub async fn delete_redirect(path: String, context: Arc<Context>) -> Result<impl warp::Reply, Infallible> {

    let resp = context.backend.delete_redirect(&path);

    match resp {
        RowChange::NotFound => ResponseMessage::from("not found").into_response(StatusCode::NOT_FOUND),
        RowChange::Value(_) => ResponseMessage::from("ok").into_response(StatusCode::OK),
        RowChange::Err(e) => {
            error!("Unable to update redirect: {:?}", e);
            ResponseMessage::from(format!("Unexpected error: {:?}", e)).into_response(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn new_redirect_json(
        ...
        .create_redirect(&info.alias, &info.destination)
    {
        RowChange::Value(result) => {
            ...
            Ok(warp::reply::with_status(warp::reply::json(&api_model), StatusCode::CREATED))
        },
        RowChange::Err(e) => {
            warn!("Unable to create redirect: {:?}", e);
            ResponseMessage::from(format!("Unable to create redirect: {:?}", e)).into_response(StatusCode::INTERNAL_SERVER_ERROR)
        }
        RowChange::NotFound => {
            warn!("Unable to create redirect");
            ResponseMessage::from("Unable to create redirect").into_response(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn list_redirects(context: Arc<Context>) -> Result<impl warp::Reply, Infallible>  {
    let resp = match context.backend.get_all(0, 10000) {
        ...
        RowChange::Err(e) => {
            ...
            return ResponseMessage::from("Unable to get redirect").into_response(StatusCode::INTERNAL_SERVER_ERROR);
        }
    };
...
pub async fn get_redirect(info: String, context: Arc<Context>) -> Result<impl warp::Reply, Infallible> {
    match context.backend.get_redirect(&info) {
        RowChange::Value(value) => {
            let redirect: ApiRedirect = value.into();
            Ok(warp::reply::with_status(warp::reply::json(&redirect), StatusCode::OK))
        },
        RowChange::NotFound => ResponseMessage::from("Unable to get redirect").into_response(StatusCode::NOT_FOUND),
        RowChange::Err(e) => {
            warn!("Unable to get redirect: {:?}", e);
            ResponseMessage::from("Unable to get redirect").into_response(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}

pub async fn find_redirect(path: warp::filters::path::Tail, context: Arc<Context>) -> Result<warp::reply::Response, Infallible> {
    ...
    match context.backend.get_redirect(&info) {
        ...
        RowChange::Err(e) => {
            warn!("Unable to get redirect: {:?}", e);
            return ResponseMessage::from("Unable to get redirect").into_response(StatusCode::INTERNAL_SERVER_ERROR).map(|x| x.into_response());
        }
    }
}
