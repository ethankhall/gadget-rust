...
use worker::*;
use hmac::{Hmac, Mac};
use jwt::{RegisteredClaims, Header, Token, VerifyWithKey};
use sha2::Sha256;
...
fn validate_auth(req: &Request, token: &Option<String>) -> Option<RegisteredClaims> {
    let auth_header = req.headers().get("Authorization").unwrap_or_default().unwrap_or_default();
...
async fn handle_create(mut req: Request, ctx: RouteContext<WorkerStore>) -> worker::Result<Response> {
    ...
    match ctx.data.create_redirect(&redirect.alias, &redirect.destination, &user).await {
        Ok(value) => Response::from_json(&value),
        Err(GadgetWorkerError::GadgetLibError(GadgetLibError::RedirectDoesNotExists(_))) => worker::Response::error("Not found", 404),
...
async fn handle_update(mut req: Request, ctx: RouteContext<WorkerStore>) -> worker::Result<Response> {
    let user = match validate_auth(&req, &ctx.data.jwt_key) {
        None => return Response::error("unauthorized", 401),
        Some(claim) => claim.subject.unwrap_or_else(|| "unknown".to_owned())
    };
    ...
    match ctx.data.update_redirect(&id, &redirect.destination, &user).await {
        Ok(value) => Response::from_json(&value),
        Err(GadgetWorkerError::GadgetLibError(GadgetLibError::RedirectDoesNotExists(_))) => worker::Response::error("Not found", 404),
        Err(e) => worker::Response::error(e.to_string(), 501),
    }
...
async fn handle_any_delete(req: Request, ctx: RouteContext<WorkerStore>) -> worker::Result<Response> {
    ...
    match ctx.data.delete_redirect(redirect_ref).await {
        Ok(_) => Response::from_json(&StatusResponse { status: "Not found".to_owned() }),
        Err(GadgetWorkerError::GadgetLibError(GadgetLibError::RedirectDoesNotExists(_))) => worker::Response::error("Not found", 404),
        Err(e) => worker::Response::error(e.to_string(), 501),
    }
}
